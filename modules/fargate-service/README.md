...
| <a name="input_existing_secrets"></a> [existing\_secrets](#input\_existing\_secrets) | Specify existing AWS Secrets Manager secrets to attach to the service containers | <pre>object({<br/>    task_def_mapping    = optional(list(object({<br/>      name: string<br/>      valueFrom: string<br/>    })), [])<br/>    kms_key_arns   = optional(list(string), [])<br/>  })</pre> | `{}` | no |
| <a name="input_health_check_grace_period_seconds"></a> [health\_check\_grace\_period\_seconds](#input\_health\_check\_grace\_period\_seconds) | The period of time, in seconds, that allows the ECS service to ignore | `number` | `null` | no |
